...
int VulkanRenderer::init(GLFWwindow* newWindow)
{
	...
	try {
		createIntstance();
		/**
		* Это первый шаг в работе с Vulkan. В этой функции вы создаете VkInstance, который является основным объектом Vulkan.
		* Он представляет ваше приложение для библиотеки Vulkan и позволяет ей взаимодействовать с драйверами GPU. 
		* Здесь вы указываете информацию о приложении (имя, версию) и запрашиваете необходимые расширения (например,
		* для интеграции с оконной системой, как GLFW) и слои валидации.
		*/

		createDebugCallback();
		/**
		* Эта функция настраивает debug callback для отладки. Она позволяет Vulkan сообщать вам об ошибках,
		* предупреждениях и других событиях, генерируемых слоями валидации. Этот callback очень
		* полезен на этапе разработки, так как помогает выявлять неправильное использование Vulkan API.
		* Важно, что он создается после VkInstance, так как для его создания нужен валидный VkInstance. 
		*/

		createSurface();
		/**
		* Здесь создается VkSurfaceKHR – абстрактный объект, который представляет поверхность окна,
		* на которую Vulkan будет выводить изображение. VkSurfaceKHR — это платформозависимый объект,
		* который связывает Vulkan с конкретным окном (например, окном GLFW), на которое будет производиться рендеринг.
		*/

		getPhysicalDevice();
		/**
		* В этой функции вы перечисляете и выбираете VkPhysicalDevice, то есть физический графический процессор (GPU)
		* в вашей системе, который будет использоваться для рендеринга. Вы проверяете его возможности, 
		* такие как поддержка необходимых расширений (например, Swap Chain), очереди команд (graphics queue, present queue)
		* и различных форматов изображений. Обычно выбирается наиболее подходящий GPU.
		*/

		createLogicalDevice();	
		/**
		* После выбора физического устройства вы создаете VkDevice (логическое устройство). 
		* Это ваше программное представление выбранного физического устройства. Через VkDevice вы 
		* будете взаимодействовать с GPU, создавая ресурсы, отправляя команды и т.д. Здесь вы 
		...
		* Swap Chain управляет набором изображений (buffers), которые используются для отображения 
		* на поверхности. Она выбирает оптимальный формат изображения, режим презентации (например, VSync)
		* и разрешение, а также определяет количество изображений, доступных для рендеринга.
		*/

		createRenderPass();
		/**
		* Здесь определяется VkRenderPass. Render Pass описывает последовательность операций 
		* рендеринга (subpasses) и используемые attachments (буферы, такие как цветовые буферы 
		* или буферы глубины/трафарета). Он указывает, как Vulkan должен обрабатывать эти attachments:
		* очищать, загружать или сохранять их содержимое в начале и конце Render Pass.
		*/

		createGraphicPipeline();
		/**
		* Эта функция создает VkPipeline – один из самых сложных объектов Vulkan. Графический пайплайн определяет
		* весь путь, который данные проходят от вершинной информации до конечных пикселей на экране. 
		* Он включает в себя конфигурацию шейдеров (vertex, fragment), настройки ввода вершин, 
		* сборки примитивов, растеризации, мультисэмплинга, смешивания цветов и другие стадии.
		*/

		createFramebuffers();
		/**
		* После создания Render Pass и Swap Chain, вы создаете VkFramebuffer. Фреймбуферы связывают 
		* Render Pass с конкретными изображениями (attachments) Swap Chain. Для каждого изображения 
		* в Swap Chain создается свой фреймбуфер, который указывает, куда именно Render Pass будет рисовать.
		*/

		createCommandPool();
		/**
		* Здесь создается VkCommandPool. Пул команд предназначен для выделения памяти для 
		...
		*/
		...
		/**
		* В этой функции вы выделяете VkCommandBuffer из созданного командного пула. 
		* Каждый командный буфер будет содержать последовательность команд рендеринга, 
		* которые GPU должен выполнить, чтобы нарисовать один кадр.
		*/

		recordCommands();
		/**
		* Это функция, где вы записываете команды рендеринга в командные буферы. 
		* Здесь вы указываете, какой Render Pass использовать, какой пайплайн применить, 
...
{
	// Wait for the fence to be signaled from the previous frame
...
void VulkanRenderer::createGraphicPipeline()
{
	...
	// How the data for a single vertex (including info such as position, colour, texture coordinates, normals etc) is represented as a whole
	VkVertexInputBindingDescription bindingDescription = {};
	bindingDescription.binding = 0;									// Can bind multiple stream of data, this defines which one
	bindingDescription.stride = sizeof(Vertex);						// Size of a single vertex obj
	...
	attributeDescriptions[0].location = 0;							// Location in shader whare data will be read from
	...
	vertexInputCreateInfo.pVertexAttributeDescriptions = attributeDescriptions.data();		// List of Vertex Attributre Descriptions (data format and where to bind to/from)
